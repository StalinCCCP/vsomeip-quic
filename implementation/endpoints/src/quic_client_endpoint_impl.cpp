...
void quic_client_endpoint_impl::connect() {
    ...
    if (!its_error || its_error == boost::asio::error::already_open) {
        ...
        if (its_error) {
            VSOMEIP_WARNING << "quic_client_endpoint_impl::connect: couldn't enable "
        ...
        socket_->set_option(boost::asio::socket_base::receive_buffer_size(
                quic_receive_buffer_size_), its_error);
        if (its_error) {
            VSOMEIP_WARNING << "quic_client_endpoint_impl::connect: couldn't set "
                    << "SO_RCVBUF: " << its_error.message()
                    << " to: " << std::dec << quic_receive_buffer_size_
        ...
        if (its_option.value() < 0
                || its_option.value() < quic_receive_buffer_size_) {
            its_error.assign(setsockopt(socket_->native_handle(),
                        SOL_SOCKET, SO_RCVBUFFORCE,
                        &quic_receive_buffer_size_, sizeof(quic_receive_buffer_size_)),
                    boost::system::generic_category());
            if (!its_error) {
                VSOMEIP_INFO << "quic_client_endpoint_impl::connect: "
    ...
    #endif
        if (its_error) {
            VSOMEIP_WARNING << "quic_client_endpoint_impl::connect: couldn't get "
                    << "SO_RCVBUF: " << its_error.message()
                    << " local port:" << std::dec << local_.port()
                     << " remote:" << get_address_port_remote();
        } else {
            VSOMEIP_INFO << "quic_client_endpoint_impl::connect: SO_RCVBUF is: "
                    << std::dec << its_option.value()
                    << " (" << quic_receive_buffer_size_ << ")"
                    << " local port:" << std::dec << local_.port()
                    << " remote:" << get_address_port_remote();
        }
        ...
        if (local_.port() != ILLEGAL_PORT) {
            ...
            if(its_bind_error) {
                VSOMEIP_WARNING << "quic_client_endpoint::connect: "
                ...
                if (its_host) {
                    ...
                    if (!its_host->on_bind_error(shared_from_this(), remote_address_, remote_port_)) {
                        ...
                        VSOMEIP_INFO << "quic_client_endpoint::connect: "
                ...
                try {
                    ...
                    VSOMEIP_ERROR << "quic_client_endpoint_impl::connect: "
        ...
        socket_->async_connect(
            ...
            strand_.wrap(
                std::bind(
                    &quic_client_endpoint_base_impl::connect_cbk,
                    shared_from_this(),
                    std::placeholders::_1
                )
            )
...
std::uint16_t quic_client_endpoint_impl::get_local_port() const {
        ...
        do {
            ...
            if (current_message_size > VSOMEIP_SOMEIP_HEADER_SIZE &&
                ...
                if (remaining_bytes - current_message_size > remaining_bytes) {
                    VSOMEIP_ERROR << "buffer underflow in quic client endpoint ~> abort!";
                    return;
...
bool quic_client_endpoint_impl::tp_segmentation_enabled(
