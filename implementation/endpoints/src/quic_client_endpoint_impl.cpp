...
nexus::global::context quic_client_endpoint_impl::global = nexus::global::init_client();
quic_client_endpoint_impl::quic_client_endpoint_impl(
    ...
    : quic_client_endpoint_base_impl(_endpoint_host, _routing_host, _local,
      ...
      quic_restart_aborts_max_(configuration_->get_max_tcp_restart_aborts()),
      quic_connect_time_max_(configuration_->get_max_tcp_connect_time()),
      aborted_restart_count_(0),
      sent_timer_(_io),
      ssl(boost::asio::ssl::context::tlsv13),
   
      //!!!socket has been moved and no more be used!!! 
      quic_client(socket_->get_executor(),_local,ssl),
      dbg(),
      //it is expected that hostname = ip addresss
      quic_conn(quic_client),
...
bool quic_client_endpoint_impl::is_local() const {
    return false;
}

void quic_client_endpoint_impl::start() {
    strand_.dispatch(std::bind(&client_endpoint_impl::connect,
            this->shared_from_this()));
}

void quic_client_endpoint_impl::restart(bool _force) {
    auto self = std::dynamic_pointer_cast< quic_client_endpoint_impl >(shared_from_this());
    auto restart_func = [self, _force] {
        if (!_force && self->state_ == cei_state_e::CONNECTING) {
            ...
            if (self->aborted_restart_count_ < self->quic_restart_aborts_max_
                    && its_connect_duration < self->quic_connect_time_max_) {
...
void quic_client_endpoint_impl::connect() {
    start_connecting_timer();
    std::lock_guard<std::mutex> its_lock(socket_mutex_);
    boost::system::error_code its_error;
    // if(!quic_conn.is_open())
    ...
    if (!its_error || its_error == boost::asio::error::already_open) {
        ...
        //it's confusing to use such option for quic version client
        ...
        if (its_device != "") {
            ...
            if (setsockopt(quic_client.local_udp_socket().native_handle(),
                    SOL_SOCKET, SO_BINDTODEVICE, its_device.c_str(), (socklen_t)its_device.size()) == -1) {
                VSOMEIP_WARNING << "QUIC Client: Could not bind to device \"" << its_device << "\"";
            }
        }
#endif
        ...
        // it's confusing for quic client to bind a port as it has already bind it
        ...
        quic_conn.async_connect(
            ...
            strand_.wrap(
                std::bind(
                    &quic_client_endpoint_base_impl::cancel_and_connect_cbk,
                    shared_from_this(),
                    std::placeholders::_1
                )
            )
        ...
        if (operations_cancelled != 0) {
            VSOMEIP_WARNING << "quic_client_endpoint::connect: Error opening socket: "
                    << its_error.message() << " remote:" << get_address_port_remote();
            strand_.post(std::bind(&quic_client_endpoint_base_impl::connect_cbk,
...
void quic_client_endpoint_impl::receive(message_buffer_ptr_t  _recv_buffer,
    ...
    if(quic_stream.is_open()) {
            ...
            if (_missing_capacity) {
                ...
                if (its_capacity < its_required_capacity) {
                    ...
                    if (_recv_buffer->size() > 1048576) {
                        VSOMEIP_INFO << "qce: recv_buffer size is: " <<
                                _recv_buffer->size()
                                << " local: " << get_address_port_local()
                                << " remote: " << get_address_port_remote();
                    }
        ...
        quic_stream.async_read_some(
            ...
            strand_.wrap(
                std::bind(
                    &quic_client_endpoint_impl::receive_cbk,
                    std::dynamic_pointer_cast< quic_client_endpoint_impl >(shared_from_this()),
                    std::placeholders::_1,
                    std::placeholders::_2,
                    _recv_buffer,
                    _recv_buffer_size
...
void quic_client_endpoint_impl::get_configured_times_from_endpoint(
...
uint16_t quic_client_endpoint_impl::get_local_port() const {
    ...
    if (quic_stream.is_open()) {
        boost::system::error_code its_error;
        endpoint_type its_local = quic_client.local_endpoint();
        if (!its_error) {
...
void quic_client_endpoint_impl::set_local_port() {
    ...
    boost::system::error_code its_error;
    if (quic_stream.is_open()) {
        endpoint_type its_endpoint = quic_client.local_endpoint();
        if (!its_error) {
            local_.port(its_endpoint.port());
        } else {
            VSOMEIP_WARNING << "quic_client_endpoint_impl::set_local_port() "
...
void quic_client_endpoint_impl::set_local_port(port_t _port) {
    ...
    if (!quic_stream.is_open()) {
        ...
        if (!its_error)
            local_.port(its_endpoint.port());
        VSOMEIP_ERROR << "quic_client_endpoint_impl::set_local_port() "
                      << "Cannot change port on open socket!";
    }
}

std::size_t quic_client_endpoint_impl::write_completion_condition(
    ...
    if (passed > send_timeout_warning_) {
        if (passed > send_timeout_) {
            ...
            VSOMEIP_WARNING << "qce::write_completion_condition: "
                    << _error.message() << "(" << std::dec << _error.value()
...
void quic_client_endpoint_impl::receive_cbk(
    ...
    if (its_host) {
        ...
        if (!_error && 0 < _bytes) {
                ...
                if (!has_full_message) {
                    if (_recv_buffer_size > VSOMEIP_RETURN_CODE_POS) {
                        ...
                        if (recv_buffer_->size() <= (its_iteration_gap + VSOMEIP_RETURN_CODE_POS)) {
                            VSOMEIP_ERROR << "QUIC Client receive_cbk is trying to access invalid vector position."
                            ...
                            VSOMEIP_ERROR << "qce: Wrong protocol version: 0x"
                    ...
                    if (max_message_size_ != MESSAGE_SIZE_UNLIMITED &&
                        ...
                        _recv_buffer->shrink_to_fit();
                        if (has_enabled_magic_cookies_) {
                            VSOMEIP_ERROR << "Received a QUIC message which exceeds "
                        ...
                        VSOMEIP_ERROR << "qce::c<" << this
                                ...
                                << " local: " << get_address_port_local()
                                << " remote: " << get_address_port_remote()
                                << ". Restarting connection due to missing/broken data QUIC stream.";
                        state_ = cei_state_e::CONNECTING;
                        shutdown_and_close_socket_unlocked(false);
                        its_lock.unlock();
            ...
            VSOMEIP_WARNING << "quic_client_endpoint receive_cbk: "
                    << _error.message() << "(" << std::dec << _error.value()
                    << ") local: " << get_address_port_local()
                    << " remote: " << get_address_port_remote();
            if (_error ==  boost::asio::error::eof ||
                ...
                if (state_ == cei_state_e::CONNECTING) {
                    VSOMEIP_WARNING << "quic_client_endpoint receive_cbk already"
                            " restarting" << get_remote_information();
                } else {
                    VSOMEIP_WARNING << "quic_client_endpoint receive_cbk restarting.";
                    state_ = cei_state_e::CONNECTING;
                    shutdown_and_close_socket_unlocked(false);
                    its_lock.unlock();
...
std::string quic_client_endpoint_impl::get_address_port_local() const {
    ...
    if (quic_stream.is_open()) {
        endpoint_type its_local_endpoint = quic_client.local_endpoint();
        if (!ec) {
...
void quic_client_endpoint_impl::handle_recv_buffer_exception(
    ...
    if (quic_stream.is_open()) {
        boost::system::error_code its_error;
        quic_stream.shutdown(socket_type::shutdown_both,its_error);
        quic_stream.close();
...
void quic_client_endpoint_impl::wait_until_sent(const boost::system::error_code &_error) {
    ...
    if (!is_sending_ || !_error) {
        ...
        sent_timer_.async_wait(std::bind(&quic_client_endpoint_impl::wait_until_sent,
                std::dynamic_pointer_cast<quic_client_endpoint_impl>(shared_from_this()),
                std::placeholders::_1));
    }
}
